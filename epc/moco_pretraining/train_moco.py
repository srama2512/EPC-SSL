...
sys.path.append("dependencies/moco")

import builder
import loader
import moco.loader
from config import get_config
from epc_dataset import EPCDataset
from epc_encoder import EPCEncoder
...
parser.add_argument(
    "-c", "--config-path", default="", type=str, help="path to model configuration"
)
parser.add_argument(
    "-j",
    "--workers",
...
parser.add_argument(
    "--remove-visual-fc",
    action="store_true",
    help="remove visual fc layers from checkpoint",
)
parser.add_argument(
    "--world-size",
    default=-1,
...
# EPC specific options
parser.add_argument(
    "--frames-per-video",
    default=4,
    type=int,
    help="number of frames to sample from video (default: 4)",
)
parser.add_argument(
    "--save-dir",
    default="./",
...
parser.add_argument(
    ...
    default=1,
    type=int,
    help="interval at which checkpoints are saved (default: 1)",
)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = builder.MoCo_EPC(
        EPCEncoder, config.MODEL, args.moco_k, args.moco_m, args.moco_t, args.mlp
    )
    print(model)
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            if args.remove_visual_fc:
                checkpoint["state_dict"] = {
                    k: v
                    for k, v in checkpoint["state_dict"].items()
                    if ("visual_fc" not in k) and (".projector." not in k)
                }
    ...
    loader_class = loader.MultiEpochsDataLoader
...
def precision(output, target, topk=(1,)):
    ...
    with torch.no_grad():
        ...
        _, pred = output.topk(maxk, 1, True, True)
        target_oh = (target > 0).float()

        pred_oh = torch.zeros_like(output)
        res = []
        for k in topk:
            pred_oh.scatter_(1, pred[:, 0:k], 1)
            prec_at_k = (pred_oh * target_oh).sum(dim=1) / k
            prec_at_k = prec_at_k.mean(dim=0, keepdim=True) * 100.0
            res.append(prec_at_k)
            pred_oh.fill_(0)
        return res
