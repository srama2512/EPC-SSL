...
class SMTStateEncoder(nn.Module):
    ...
    def __init__(
        ...
        use_goal_encoding: bool = False,
        goal_dims: Optional[int] = None,
    ):
        ...
        self._use_goal_encoding = use_goal_encoding
        self._goal_dims = goal_dims
        ...
        if self._use_goal_encoding:
            self.goal_observation_mlp = nn.Sequential(
                nn.Linear(goal_dims + input_size, input_size),
                nn.ReLU(),
    ...
    def hidden_state_size(self):
        if self._use_goal_encoding:
            return self._dim_feedforward + self._goal_dims
        else:
    ...
    def _compute_relative_pose(self, pose_a, pose_b):
        ...
        phi_ab = torch.atan2(
            pose_b[..., 1] - pose_a[..., 1], pose_b[..., 0] - pose_a[..., 0]
        )
